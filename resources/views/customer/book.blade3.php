...
<body class="bg-gray-50" x-data="bookingWizard()" x-init="init()">
    ...
    <div class="container px-4 mx-auto mt-4">
        <div class="max-w-5xl mx-auto p-4 border-2 border-blue-200 bg-blue-50 rounded-xl">
            <div class="flex items-start gap-3">
                <svg class="flex-shrink-0 w-6 h-6 text-blue-600 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                </svg>
                <div class="flex-1">
                    <h4 class="font-bold text-blue-900 mb-1">{{ __('advance_payment.advance_required') }}</h4>
                    <p class="text-sm text-blue-800 mb-2">
                        ...
                        @else
                            {{ __('advance_payment.advance_payment_info', [
                                ...
                                'balance' => number_format($hall->price_per_slot - (($hall->price_per_slot * $hall->advance_payment_percentage) / 100), 3)
                            ]) }}
                        @endif
                    </p>
    ...
    @if($errors->any())
        <div class="container px-4 mx-auto mt-4">
            <div class="max-w-5xl p-4 mx-auto border-2 border-red-200 bg-red-50 rounded-xl">
                ...
                <ul class="space-y-1 text-sm list-disc list-inside text-red-700 {{ app()->getLocale() === 'ar' ? 'mr-6' : 'ml-6' }}">
        ...
        <form action="{{ route('customer.booking.store', $hall->slug) }}" method="POST" id="bookingForm">
            ...
            <div class="grid max-w-5xl grid-cols-1 gap-6 mx-auto lg:grid-cols-3">
                ...
                <div class="space-y-6 lg:col-span-2">
                    ...
                    <div x-show="currentStep === 0" x-transition class="p-6 bg-white border border-gray-200 shadow-sm slide-in rounded-2xl md:p-8">
                        ...
                        <!-- Date Selection -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.event_date') }} <span class="text-red-500">*</span></label>
                            <input
                                type="date"
                                name="booking_date"
                                ...
                                :min="minDate"
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition"
                                required>
                        </div>

                        <!-- Time Slot Selection -->
                        <div class="mb-6">
                            <label class="block mb-3 text-sm font-semibold text-gray-700">{{ __('halls.time_slot') }} <span class="text-red-500">*</span></label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative">
                                    <input type="radio" name="time_slot" value="morning" x-model="formData.time_slot" @change="checkAvailability()" class="sr-only peer" required>
                                    <div class="p-4 text-center transition border-2 border-gray-300 cursor-pointer rounded-xl peer-checked:border-primary-500 peer-checked:bg-primary-50 hover:border-primary-300">
                                ...
                                <label class="relative">
                                    <input type="radio" name="time_slot" value="afternoon" x-model="formData.time_slot" @change="checkAvailability()" class="sr-only peer">
                                    <div class="p-4 text-center transition border-2 border-gray-300 cursor-pointer rounded-xl peer-checked:border-primary-500 peer-checked:bg-primary-50 hover:border-primary-300">
                                ...
                                <label class="relative">
                                    <input type="radio" name="time_slot" value="evening" x-model="formData.time_slot" @change="checkAvailability()" class="sr-only peer">
                                    <div class="p-4 text-center transition border-2 border-gray-300 cursor-pointer rounded-xl peer-checked:border-primary-500 peer-checked:bg-primary-50 hover:border-primary-300">
                                        <div class="text-2xl mb-1">🌙</div>
                                ...
                                <label class="relative">
                                    <input type="radio" name="time_slot" value="full_day" x-model="formData.time_slot" @change="checkAvailability()" class="sr-only peer">
                                    <div class="p-4 text-center transition border-2 border-gray-300 cursor-pointer rounded-xl peer-checked:border-primary-500 peer-checked:bg-primary-50 hover:border-primary-300">
                                        <div class="text-2xl mb-1">⏰</div>
                            ...
                            <div :class="{
                                ...
                                <template x-if="!availabilityChecking">
                                    <svg class="w-5 h-5" :class="{
                                        ...
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" x-bind:d="isAvailable ? 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' : 'M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z'"></path>
                                    </svg>
                                </template>
                        ...
                        <!-- Number of Guests -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.number_of_guests') }} <span class="text-red-500">*</span></label>
                            <input
                                type="number"
                                ...
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:border-primary-500 focus:ring-2 focus:ring-primary-200 transition"
                                required>
                            <p class="mt-1 text-xs text-gray-500">{{ __('halls.capacity_range', ['min' => $hall->capacity_min, 'max' => $hall->capacity_max]) }}</p>
                        </div>

                        <!-- Event Type -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.event_type') }}</label>
                            <select
                                name="event_type"
                        ...
                        <div>
                            <label class="block mb-3 text-sm font-semibold text-gray-700">{{ __('halls.extra_services') }}</label>
                            <div class="space-y-3">
                                @foreach($hall->activeExtraServices as $service)
                                <label class="flex items-start gap-3 p-4 transition border-2 border-gray-200 cursor-pointer rounded-xl hover:border-primary-300">
                                    <input
                                        type="checkbox"
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <span class="font-bold text-primary-600">{{ number_format($service->price, 3) }} OMR</span>
                                        </div>
                    ...
                    <div x-show="currentStep === 1" x-transition class="p-6 bg-white border border-gray-200 shadow-sm slide-in rounded-2xl md:p-8">
                        <h2 class="flex items-center gap-2 mb-6 text-2xl font-bold text-gray-900">
                            <svg class="w-7 h-7 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Name -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.full_name') }} <span class="text-red-500">*</span></label>
                            <input
                                type="text"
                                name="customer_name"
                        ...
                        <!-- Email -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.email') }} <span class="text-red-500">*</span></label>
                            <input
                                type="email"
                                name="customer_email"
                        ...
                        <!-- Phone -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.phone') }} <span class="text-red-500">*</span></label>
                            <input
                                type="tel"
                                name="customer_phone"
                        ...
                        <!-- Notes -->
                        <div class="mb-6">
                            <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.additional_notes') }}</label>
                            <textarea
                                name="customer_notes"
                    ...
                    <!-- Step 3: Review & Confirm (Keep existing but continued in next part due to length...) -->
                    <!-- I'll continue with the complete file in the outputs -->
                    ...
                    <div class="sticky p-6 bg-white border border-gray-200 shadow-sm top-24 rounded-2xl">
                        ...
                        <div class="py-4 space-y-3 border-t border-gray-200">
                            ...
                            <div x-show="servicesTotal > 0" class="flex justify-between text-sm">
                                ...
                                <span class="font-medium text-gray-900" x-text="servicesTotal.toFixed(3) + ' OMR'"></span>
                            </div>
                        </div>
                        ...
                        <div class="p-4 mt-4 border-2 border-blue-200 bg-blue-50 rounded-xl">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <span class="text-sm font-bold text-blue-900">{{ __('advance_payment.payment_type') }}</span>
                            </div>
                            @if($hall->advance_payment_type === 'fixed')
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-blue-700">{{ __('advance_payment.customer_pays_advance') }}:</span>
                                    <span class="font-bold text-blue-900">{{ number_format($hall->advance_payment_amount, 3) }} OMR</span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-blue-700">{{ __('advance_payment.balance_due') }}:</span>
                                    <span class="font-semibold text-blue-800" x-text="(total - {{ $hall->advance_payment_amount }}).toFixed(3) + ' OMR'"></span>
                                </div>
                            </div>
                            @else
                            <div class="space-y-2 text-sm">
                                <div class="flex justify-between">
                                    <span class="text-blue-700">{{ __('advance_payment.customer_pays_advance') }} ({{ $hall->advance_payment_percentage }}%):</span>
                                    <span class="font-bold text-blue-900" x-text="(total * {{ $hall->advance_payment_percentage }} / 100).toFixed(3) + ' OMR'"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-blue-700">{{ __('advance_payment.balance_due') }}:</span>
                                    <span class="font-semibold text-blue-800" x-text="(total - (total * {{ $hall->advance_payment_percentage }} / 100)).toFixed(3) + ' OMR'"></span>
                                </div>
                            </div>
                            @endif
                        </div>
