...
<body class="font-tajawal bg-gray-50" x-data="hallsApp()" x-cloak>
    ...
    <section class="relative overflow-hidden text-white bg-gradient-to-br from-gray-900 via-gray-800 to-brand-900">
        ...
        <div class="absolute inset-0 opacity-[0.07]"
            style="background-image: url('data:image/svg+xml,%3Csvg width=&quot;40&quot; height=&quot;40&quot; viewBox=&quot;0 0 40 40&quot; xmlns=&quot;http://www.w3.org/2000/svg&quot;%3E%3Cg fill=&quot;%23fff&quot; fill-opacity=&quot;1&quot;%3E%3Cpath d=&quot;M20 20.5V18H0v-2h20v-2l2 3.5-2 3zM0 20h2v2H0v-2z&quot;/%3E%3C/g%3E%3C/svg%3E');">
        </div>
            ...
            <form id="smart-search-form" action="{{ route('customer.halls.index') }}" method="GET"
                ...
                <div class="grid grid-cols-2 gap-3 md:grid-cols-12">
                    ...
                    <div class="col-span-1 md:col-span-3">
                        <label
                            class="block text-[10px] md:text-xs font-semibold text-brand-300 uppercase tracking-wider mb-1">
                        ...
                        <div class="relative">
                            ...
                            <div id="date-badge"
                                class="hidden absolute -top-2 {{ app()->getLocale() === 'ar' ? '-left-2' : '-right-2' }} text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-3">
                        ...
                        <select id="search-time-slot" name="time_slot" required
                            ...
                            <option value="" disabled {{ request('time_slot') ? '' : 'selected' }}>
                                {{ __('halls.select_time') }}</option>
                            @foreach ($timeSlots as $val => $lbl)
                                <option value="{{ $val }}"
                                    {{ request('time_slot') === $val ? 'selected' : '' }}>{{ __($lbl) }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select id="search-region" name="region_id"
                            ...
                            @foreach ($regions as $region)
                                <option value="{{ $region->id }}"
                                    {{ (int) request('region_id') === $region->id ? 'selected' : '' }}>
                                    {{ $region->name }}</option>
                            @endforeach
                        </select>
                    </div>
                        ...
                        <select id="search-city" name="city_id"
                            ...
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}"
                                    {{ (int) request('city_id') === $city->id ? 'selected' : '' }}>{{ $city->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div x-data="{ open: {{ request('max_price') || request('features') || request('min_guests') || request('search') ? 'true' : 'false' }} }" class="mt-3">
                    <button type="button" @click="open = !open"
                        ...
                        <span
                            x-text="open ? '{{ __('halls.hide_filters') }}' : '{{ __('halls.more_filters') }}'"></span>
                    </button>
                    <div x-show="open" x-collapse class="grid grid-cols-2 gap-3 mt-3 md:grid-cols-4">
                        {{-- Text Search --}}
                        <div class="col-span-2 md:col-span-1">
                            <label
                                class="block text-[10px] md:text-xs font-semibold text-brand-300 uppercase tracking-wider mb-1">{{ __('halls.search_placeholder') }}</label>
                        ...
                        <div>
                            <label
                                class="block text-[10px] md:text-xs font-semibold text-brand-300 uppercase tracking-wider mb-1">{{ __('halls.guests') }}</label>
                            <input type="number" name="min_guests" value="{{ request('min_guests') }}"
                                min="1" placeholder="{{ __('halls.any') }}"
                                class="w-full px-3 py-2.5 bg-white/90 text-gray-800 rounded-xl border-0 focus:ring-2 focus:ring-brand-400 placeholder-gray-400 font-medium text-sm">
                        </div>
                        {{-- Max Price --}}
                        <div>
                            <label
                                class="block text-[10px] md:text-xs font-semibold text-brand-300 uppercase tracking-wider mb-1">{{ __('halls.max_price') }}
                        ...
                        <div class="col-span-2 md:col-span-1">
                            <label
                                class="block text-[10px] md:text-xs font-semibold text-brand-300 uppercase tracking-wider mb-1">{{ __('halls.features') }}</label>
                            <div class="flex flex-wrap gap-1.5 max-h-20 overflow-y-auto p-1.5 bg-white/5 rounded-xl">
                                @foreach ($features as $feature)
                                    ...
                                    <label
                                        class="inline-flex items-center gap-1 px-2 py-1 rounded-md cursor-pointer transition text-xs {{ $fc ? 'bg-brand-500 text-white' : 'bg-white/20 text-white/80 hover:bg-white/30' }}">
                                        <input type="checkbox" name="features[]" value="{{ $feature->id }}"
                                            class="sr-only" {{ $fc ? 'checked' : '' }}>
                ...
                @if (isset($stats) && $stats['min_price'])
                    <p class="mt-2 text-[11px] text-brand-300/60 text-center">
                        ...
                        {{ number_format($stats['max_price'], 3) }} <img src="{{ asset('images/Medium.svg') }}"
                            alt="Omani Riyal" class="inline w-5 h-5 -mt-1">
                    </p>
                @endif
            </form>
        </div>
            ...
            <div class="flex-1 min-w-0">
                @if ($isDateSearch)
                    <h2 class="text-lg font-bold text-gray-800 truncate">
                        @if ($availableCount > 0)
                            <span class="text-brand-600">{{ $availableCount }}</span>
                            {{ __('halls.halls_available') }}
                        @else
            ...
            <div class="items-center hidden gap-2 md:flex">
                ...
                <div class="flex items-center gap-1 p-1 bg-gray-100 rounded-xl">
                    <button @click="view = 'grid'" :class="view === 'grid' ? 'bg-white shadow-sm' : ''"
                        ...
                        <svg class="w-5 h-5" :class="view === 'grid' ? 'text-brand-600' : 'text-gray-500'"
                    ...
                    <button @click="view = 'map'" :class="view === 'map' ? 'bg-white shadow-sm' : ''"
                        class="p-2 transition rounded-lg">
                        <svg class="w-5 h-5" :class="view === 'map' ? 'text-brand-600' : 'text-gray-500'"
                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <select id="sort-select" onchange="updateSort(this.value)"
                    ...
                    <option value="" {{ !request('sort') ? 'selected' : '' }}>{{ __('halls.best_match') }}
                    </option>
                    ...
                    <option value="capacity" {{ request('sort') === 'capacity' ? 'selected' : '' }}>
                        {{ __('halls.largest_capacity') }}</option>
                </select>
            </div>
        </div>
    </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                @foreach ($suggestions as $s)
                                    <a href="{{ route('customer.halls.index', array_merge(request()->except('date'), ['date' => $s['date']])) }}"
                                        ...
                                        <div>
                                            <div class="font-semibold text-gray-800 group-hover:text-brand-700">
                                                {{ $s['formatted'] }}</div>
                                            <div class="text-[10px] text-gray-500">{{ $s['hall_count'] }}
                                                {{ __('halls.available') }}</div>
                                        </div>
                                        <svg class="w-4 h-4 text-gray-300 group-hover:text-brand-500 {{ app()->getLocale() === 'ar' ? 'rotate-180' : '' }}"
                                            fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 5l7 7-7 7" />
                                        </svg>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-0.5"
                            x-text="activeFiltersCount > 0 ? activeFiltersCount + ' {{ __('halls.active_filters') }}' : '{{ __('halls.no_active_filters') }}'">
                        </p>
                    </div>
            ...
            <form action="{{ route('customer.halls.index') }}" method="GET">
                <div class="filter-body">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-bold text-gray-700">{{ __('halls.region') }}</label>
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-bold text-gray-700">{{ __('halls.guests') }}</label>
                            <input type="number" name="min_guests" value="{{ request('min_guests') }}"
                                min="1" placeholder="{{ __('halls.enter_guests') }}"
                                class="w-full px-4 text-base border-2 border-gray-200 h-14 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                        </div>
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-bold text-gray-700">{{ __('halls.price_range') }}</label>
                            ...
                            @if (isset($stats) && $stats['min_price'])
                                <p class="mt-2 text-xs text-gray-500">{{ __('halls.starting_from') }}
                                    {{ number_format($stats['min_price'], 3) }} –
                                    {{ number_format($stats['max_price'], 3) }} <img
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-bold text-gray-700">{{ __('halls.view_mode') }}</label>
                            <div class="grid grid-cols-2 gap-3">
                                <label
                                    class="flex items-center justify-center px-4 transition border-2 cursor-pointer h-14 rounded-xl"
                                    :class="!mobileFilters.view_mode ? 'border-brand-500 bg-brand-50 text-brand-700' :
                                        'border-gray-200'">
                                ...
                                <label
                                    ...
                                    :class="mobileFilters.view_mode === 'by_region' ?
                                        'border-brand-500 bg-brand-50 text-brand-700' : 'border-gray-200'">
                        ...
                        <div>
                            <label
                                class="block mb-2 text-sm font-bold text-gray-700">{{ __('halls.sort_by') }}</label>
                            <select name="sort"
                                class="w-full px-4 text-base border-2 border-gray-200 h-14 rounded-xl focus:ring-2 focus:ring-brand-500 focus:border-transparent">
                                <option value="" {{ !request('sort') ? 'selected' : '' }}>
                                    {{ __('halls.best_match') }}</option>
                                <option value="rating" {{ request('sort') === 'rating' ? 'selected' : '' }}>
                                    {{ __('halls.highest_rated') }}</option>
                                <option value="price_low" {{ request('sort') === 'price_low' ? 'selected' : '' }}>
                                    {{ __('halls.price_low_high') }}</option>
    ...
    <nav class="bottom-nav md:hidden">
        <div class="grid grid-cols-3 gap-1 p-2">
            <button @click="view = 'grid'" :class="view === 'grid' ? 'text-brand-600' : 'text-gray-500'"
                class="flex flex-col items-center gap-1 p-2 rounded-lg active:bg-gray-100">
            ...
            <button @click="view = 'map'" :class="view === 'map' ? 'text-brand-600' : 'text-gray-500'"
                class="flex flex-col items-center gap-1 p-2 rounded-lg active:bg-gray-100">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7" />
            ...
            <button @click="showFilters = true"
                class="relative flex flex-col items-center gap-1 p-2 text-gray-500 rounded-lg active:bg-gray-100">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        function hallsApp() {
            return {
                ...
                initMap() {
                    ...
                    if (coords.length > 1) this.map.fitBounds(coords, {
                        padding: [30, 30]
                    });
                }
            };
                    ...
                    fetch(`{{ url('/halls/cities') }}/${this.value}`)
                        ...
                        .then(cities => {
                            ...
                            cities.forEach(c => {
                                ...
                                opt.value = c.id;
                                opt.textContent = typeof c.name === 'object' ? (c.name[loc] || c
                                    .name.en || '') : c.name;
                ...
                timer = setTimeout(() => {
                    ...
                    fetch(`/api/halls/check-availability?${p}`)
                        ...
                        .then(d => {
                            ...
                            badge.className =
                                `absolute -top-2 {{ app()->getLocale() === 'ar' ? '-left-2' : '-right-2' }} text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full shadow-lg ${d.available_count > 0 ? 'bg-brand-500' : 'bg-red-500'}`;
