...
<body class="bg-gray-50" x-data="hallsApp()">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <a href="{{ route('filament.admin.auth.login') }}"
                       class="items-center hidden gap-2 px-4 py-2 text-white transition rounded-lg shadow-sm md:flex bg-primary-600 hover:bg-primary-700">
                        ...
                        <span class="text-sm font-medium">دخول</span>
                    </a>
                </div>
            </div>
        </div>
    ...
    <div class="sticky z-40 bg-white border-b border-gray-200 shadow-md top-16">
        <div class="container px-4 py-3 mx-auto">
            <form action="{{ route('customer.halls.index') }}" method="GET" class="flex items-center gap-2">
                ...
                <button type="submit" class="items-center hidden h-12 gap-2 px-6 font-medium text-white transition shadow-sm md:flex bg-primary-600 rounded-xl hover:bg-primary-700">
                ...
                <button
                    ...
                    <span x-show="activeFiltersCount > 0" class="absolute -top-2 {{ app()->getLocale() === 'ar' ? '-left-2' : '-right-2' }} bg-red-500 text-white text-xs font-bold rounded-full w-6 h-6 flex items-center justify-center shadow-lg" x-text="activeFiltersCount"></span>
                </button>
            </form>
        </div>
    </div>
                ...
                <div class="sticky space-y-4 top-32">
                    <div class="p-6 bg-white border border-gray-200 shadow-sm rounded-2xl">
                        ...
                        <form action="{{ route('customer.halls.index') }}" method="GET" class="space-y-5">
                            ...
                            <!-- Region -->
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.region') }}</label>
                                <select
                                    name="region_id"
                            ...
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.city') }}</label>
                                <select
                                    name="city_id"
                                    onchange="this.form.submit()"
                                    class="w-full h-12 px-4 transition border-2 border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                            ...
                            <!-- Capacity -->
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.guests') }}</label>
                                <input
                                    type="number"
                                    name="capacity"
                            ...
                            <!-- Price Range -->
                            <div>
                                <label class="block mb-2 text-sm font-semibold text-gray-700">{{ __('halls.price_range') }}</label>
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
                    <div class="flex items-center gap-3">
                        <span class="text-sm font-medium text-gray-600">
                            <strong class="text-primary-600">{{ $halls->total() }}</strong> {{ __('halls.halls_found') }}
                        </span>
                    ...
                    @if(request('view_mode') === 'by_region')
                        ...
                        @foreach($regions as $region)
                            ...
                            @if($regionHalls->count() > 0)
                                ...
                                <div class="mb-12">
                                    <div class="flex items-center justify-between pb-3 mb-6 border-b-2 border-primary-100">
                ...
                <div x-show="view === 'map'" x-cloak class="pb-24 md:pb-0">
                    ...
                    <div class="p-4 mt-4 border-2 border-blue-200 bg-blue-50 rounded-xl">
                        <p class="text-sm font-medium text-blue-800">
                            <svg class="w-5 h-5 inline {{ app()->getLocale() === 'ar' ? 'ml-1' : 'mr-1' }}" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div
            ...
            <form action="{{ route('customer.halls.index') }}" method="GET">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            <label class="block mb-3 text-sm font-bold text-gray-700">
                                <svg class="w-5 h-5 inline {{ app()->getLocale() === 'ar' ? 'ml-1' : 'mr-1' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <select
                                name="region_id"
                                x-model="mobileFilters.region_id"
                                @change="loadCitiesMobile()"
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="flex items-center justify-center px-4 transition border-2 cursor-pointer h-14 rounded-xl" :class="!mobileFilters.view_mode ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 hover:border-gray-300'">
                                ...
                                <label class="flex items-center justify-center px-4 transition border-2 cursor-pointer h-14 rounded-xl" :class="mobileFilters.view_mode === 'by_region' ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-gray-200 hover:border-gray-300'">
                ...
                <div class="filter-footer">
                    <div class="grid grid-cols-2 gap-3">
                        <a
                            href="{{ route('customer.halls.index') }}?lang={{ app()->getLocale() }}"
    ...
    <nav class="bottom-nav md:hidden">
        <div class="grid grid-cols-3 gap-1 p-2">
            <button
                @click="view = 'grid'"
                :class="view === 'grid' ? 'text-primary-600' : 'text-gray-600'"
            ...
            <button
                @click="view = 'map'"
                :class="view === 'map' ? 'text-primary-600' : 'text-gray-600'"
                class="flex flex-col items-center gap-1 p-2 transition rounded-lg active:bg-gray-100">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"></path>
                </svg>
            ...
            <button
                ...
                <span x-show="activeFiltersCount > 0" class="absolute top-0 flex items-center justify-center w-5 h-5 text-xs font-bold text-white bg-red-500 rounded-full shadow-lg right-2" x-text="activeFiltersCount"></span>
            </button>
        </div>
    </nav>
