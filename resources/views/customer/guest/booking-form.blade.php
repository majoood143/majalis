...
@section('content')
        ...
        <div class="max-w-4xl px-4 mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-center space-x-4 rtl:space-x-reverse">
                    ...
                    <div class="flex items-center">
                        <span
                            class="flex items-center justify-center w-8 h-8 text-sm font-medium text-gray-600 bg-gray-300 rounded-full">
                            4
                        </span>
                    ...
                    <div class="overflow-hidden bg-white shadow-sm rounded-xl">
                        ...
                        <div class="p-6">
                            ...
                            <div class="p-4 mb-6 border border-blue-200 rounded-lg bg-blue-50">
                                <h3 class="mb-2 font-medium text-blue-900">{{ __('guest.form_your_information') }}</h3>
                                <div class="space-y-1 text-sm text-blue-800">
                                    <p><span class="font-medium">{{ __('guest.details_label_guest_name') }}:</span>
                                        {{ $guestSession->name }}</p>
                                    <p><span class="font-medium">{{ __('guest.details_label_guest_email') }}:</span>
                                        {{ $guestSession->email }}</p>
                                    <p><span class="font-medium">{{ __('guest.details_label_guest_phone') }}:</span>
                                        {{ $guestSession->phone }}</p>
                                </div>
                            </div>
                            ...
                            <form method="POST"
                                ...
                                <div class="mb-5">
                                    <label for="booking_date" class="block mb-1 text-sm font-medium text-gray-700">
                                        {{ __('guest.details_label_date') }} <span class="text-red-500">*</span>
                                    </label>
                                    <input type="date" id="booking_date" name="booking_date" x-model="bookingDate"
                                        @change="checkAvailability()" min="{{ now()->addDay()->format('Y-m-d') }}"
                                ...
                                <div class="mb-5">
                                    <label class="block mb-2 text-sm font-medium text-gray-700">
                                        {{ __('guest.details_label_time') }} <span class="text-red-500">*</span>
                                    </label>
                                    <div class="grid grid-cols-2 gap-3 sm:grid-cols-4">
                                        @php
                                            $timeSlots = [
                                                'morning' => ['label' => __('guest.time_slot_morning'), 'icon' => '🌅'],
                                                'afternoon' => [
                                                    'label' => __('guest.time_slot_afternoon'),
                                                    'icon' => '☀️',
                                                ],
                                                'evening' => ['label' => __('guest.time_slot_evening'), 'icon' => '🌙'],
                                                'full_day' => [
                                                    'label' => __('guest.time_slot_full_day'),
                                                    'icon' => '📅',
                                                ],
                                            ];
                                        @endphp
                                        ...
                                        @foreach ($timeSlots as $value => $slot)
                                            <label class="relative">
                                                <input type="radio" name="time_slot" value="{{ $value }}"
                                                    x-model="timeSlot" @change="checkAvailability()" class="sr-only peer"
                                                    {{ old('time_slot') === $value ? 'checked' : '' }}>
                                                <div
                                                    class="p-3 text-center transition border-2 rounded-lg cursor-pointer peer-checked:border-primary-500 peer-checked:bg-primary-50 hover:bg-gray-50">
                                    ...
                                    <div x-show="availabilityMessage" class="mt-3">
                                        <p :class="isAvailable ? 'text-green-600' : 'text-red-600'"
                                ...
                                <div class="mb-5">
                                    <label for="number_of_guests" class="block mb-1 text-sm font-medium text-gray-700">
                                        {{ __('guest.details_label_guests') }} <span class="text-red-500">*</span>
                                    </label>
                                    ...
                                    <p class="mt-1 text-xs text-gray-500">
                                        {{ __('guest.form_capacity') }}: {{ $hall->capacity_min }} -
                                        {{ $hall->capacity_max }} {{ __('guest.form_guests_label') }}
                                    </p>
                                ...
                                <div class="mb-5">
                                    <label for="event_type" class="block mb-1 text-sm font-medium text-gray-700">
                                        {{ __('guest.details_label_event_type') }}
                                    </label>
                                ...
                                @if ($hall->activeExtraServices->count() > 0)
                                    <div class="mb-5">
                                        <label class="block mb-2 text-sm font-medium text-gray-700">
                                            {{ __('guest.details_label_services') }}
                                        </label>
                                        <div class="space-y-3">
                                                    ...
                                                    <div class="flex-1 ms-3">
                                                        <div class="flex justify-between">
                                                            ...
                                                            <span class="font-medium text-primary-600">
                                                                {{ number_format($service->price, 3) }} <img
                                                                    src="{{ asset('images/Medium.svg') }}"
                                ...
                                <div class="mb-5">
                                    <label for="customer_notes" class="block mb-1 text-sm font-medium text-gray-700">
                                        {{ __('guest.details_label_special_requests') }}
                                    </label>
                                    <textarea id="customer_notes" name="customer_notes" rows="3"
                                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                                        placeholder="{{ __('guest.form_special_requests_placeholder') }}">{{ old('customer_notes') }}</textarea>
                                </div>
                                ...
                                {{-- Submit Button --}}
                                <button type="submit" :disabled="!isAvailable || !agreeTerms || isSubmitting"
                ...
                <div class="lg:col-span-1">
                    <div class="sticky overflow-hidden bg-white shadow-sm rounded-xl top-4">
                        <div class="px-4 py-3 border-b border-gray-200 bg-gray-50">
                            <h3 class="font-semibold text-gray-900">{{ __('guest.form_booking_summary') }}</h3>
                        </div>
                            ...
                            <div class="flex items-start mb-4 space-x-3 rtl:space-x-reverse">
                                ...
                                <div>
                                    <h4 class="font-medium text-gray-900">
                                        {{ $hall->getTranslation('name', app()->getLocale()) }}</h4>
                                    <p class="text-sm text-gray-500">
                                        {{ $hall->city?->getTranslation('name', app()->getLocale()) }}</p>
                                </div>
                            </div>
            ...
            <div class="mt-8 text-center">
                <a href="{{ route('guest.book', ['hall' => $hall->slug, 'lang' => app()->getLocale()]) }}"
    ...
    @php
        ...
        $servicesForJs = $hall->activeExtraServices
            ->map(function ($s) {
                return [
                    'id' => $s->id,
