    ...
    <div class="flex items-start justify-between border-b pb-3">
        ...
        @php
            $badgeClass = match($record->type) {
                App\Models\TicketMessageType::STAFF_REPLY    => 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100',
                App\Models\TicketMessageType::INTERNAL_NOTE  => 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100',
    ...
    @if($record->has_attachments)
        <div class="border-t pt-3">
            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2">
                {{ __('ticket_admin.blade_attachments', ['count' => $record->attachments_count]) }}
            </h4>
                    ...
                    <div class="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-800 rounded-lg">
                        <div class="flex items-center space-x-2">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-900 dark:text-white">
                                    {{ $attachment['original_name'] ?? __('ticket_admin.blade_attachment_label', ['number' => $index + 1]) }}
                                </p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        <a href="{{ $record->getAttachmentDownloadUrl($index) }}"
                           ...
                           download>
                            ...
                            {{ __('ticket_admin.blade_download') }}
                        </a>
                    </div>
                @endforeach
            </div>
        ...
        <div class="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            <div class="flex items-center space-x-4">
                <span class="flex items-center">
                    ...
                    {{ __('ticket_admin.blade_posted') }} {{ $record->created_at->diffForHumans() }}
                </span>
                ...
                @if($record->is_read)
                    <span class="flex items-center text-green-600 dark:text-green-400">
                        ...
                        {{ __('ticket_admin.blade_read') }} {{ $record->read_at?->diffForHumans() }}
                    </span>
                @else
                ...
                @if($record->is_internal)
                    <span class="flex items-center text-yellow-600 dark:text-yellow-400">
                        ...
                        {{ __('ticket_admin.blade_internal_only') }}
                    </span>
                @endif
            </div>
