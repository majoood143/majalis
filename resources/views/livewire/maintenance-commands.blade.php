<div>
        ...
        <div class="grid grid-cols-1 gap-4 mb-6 md:grid-cols-2">
            <div class="space-y-3">
                <div class="flex justify-between">
                    ...
                    <span
                        class="text-sm font-medium text-gray-900 dark:text-white">{{ $systemInfo['laravel_version'] }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-sm text-gray-600 dark:text-gray-400">PHP Version</span>
                    <span
                        class="text-sm font-medium text-gray-900 dark:text-white">{{ $systemInfo['php_version'] }}</span>
                </div>
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium">
                        @if ($systemInfo['environment'] === 'production')
                            <span
                                class="px-2 py-1 text-xs text-red-800 bg-red-100 rounded-full dark:bg-red-900 dark:text-red-200">Production</span>
                        @elseif($systemInfo['environment'] === 'local')
                            <span
                                class="px-2 py-1 text-xs text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">Local</span>
                        @else
                            <span
                                class="px-2 py-1 text-xs text-yellow-800 bg-yellow-100 rounded-full dark:bg-yellow-900 dark:text-yellow-200">{{ $systemInfo['environment'] }}</span>
                        @endif
                    </span>
                </div>

                <div class="flex justify-between">
                    <span class="text-sm text-gray-600 dark:text-gray-400">Queue Driver</span>
                    <span class="text-sm font-medium text-gray-900 dark:text-white">
                        {{ $systemInfo['queue_driver'] }} ({{ $systemInfo['queue_connection'] }})
                    </span>
                </div>
            </div>
                ...
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium">
                        @if ($systemInfo['debug_mode'])
                            <span
                                class="px-2 py-1 text-xs text-red-800 bg-red-100 rounded-full dark:bg-red-900 dark:text-red-200">Enabled</span>
                        @else
                            <span
                                class="px-2 py-1 text-xs text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">Disabled</span>
                        @endif
                    </span>
                </div>
                <div class="flex justify-between">
                    ...
                    <span class="text-sm font-medium">
                        @if ($systemInfo['maintenance_mode'])
                            <span
                                class="px-2 py-1 text-xs text-red-800 bg-red-100 rounded-full dark:bg-red-900 dark:text-red-200">Active</span>
                        @else
                            <span
                                class="px-2 py-1 text-xs text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">Inactive</span>
                        @endif
                    </span>
                </div>
                <div class="flex justify-between">
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <div class="p-6 mt-6 bg-white border border-gray-200 shadow-sm rounded-xl dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-3">
                    ...
                    <button wire:click="clearAppCache" wire:loading.attr="disabled"
        ...
        <div class="p-6 mt-6 bg-white border border-gray-200 shadow-sm rounded-xl dark:border-gray-700 dark:bg-gray-800">
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    <div
                        class="flex items-center justify-center w-10 h-10 rounded-lg bg-amber-100 dark:bg-amber-900/30">
    ...
    <div class="grid gap-6 mb-6 md:grid-cols-2">
        ...
        <div class="p-6 mt-6 bg-white border border-gray-200 shadow-sm rounded-xl dark:border-gray-700 dark:bg-gray-800">
            ...
            <div class="grid grid-cols-1 gap-4">
                <!-- Clear All Caches -->
                <button wire:click="runAllCommands" wire:loading.attr="disabled"
                ...
                <!-- Clear Logs -->
                <button wire:click="clearLogs" wire:loading.attr="disabled"
                    class="inline-flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-gray-700 transition bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600">
                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd"
                            d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
    ...
    <!-- Output Section -->
    <div class="p-6 mt-6 bg-white border border-gray-200 shadow-sm mt rounded-xl dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3">
            ...
            <button type="button" wire:click="$set('output', '')"
        ...
        <div class="p-4 font-mono text-sm bg-gray-900 rounded-lg dark:text-white dark:bg-gray-900">
    ...
    @if (count($commandHistory) > 0)
        <div
            class="p-6 mt-6 bg-white border border-gray-200 shadow-sm rounded-xl dark:border-gray-700 dark:bg-gray-800">
            <div class="flex items-center gap-3 mb-4">
                <div class="flex items-center justify-center w-10 h-10 bg-indigo-100 rounded-lg dark:bg-indigo-900/30">
                        ...
                        @foreach (array_reverse($commandHistory) as $history)
                            <tr>
                                ...
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                    {{ $history['timestamp'] }}</td>
                                <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                    {{ $history['execution_time'] }}s</td>
                                <td class="px-4 py-3 text-sm">
                                    @if ($history['status'] === 'success')
                                        <span
                                            class="px-2 py-1 text-xs text-green-800 bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">Success</span>
                                    @elseif($history['status'] === 'error')
                                        <span
                                            class="px-2 py-1 text-xs text-red-800 bg-red-100 rounded-full dark:bg-red-900 dark:text-red-200">Error</span>
                                    @else
                                        <span
                                            class="px-2 py-1 text-xs text-yellow-800 bg-yellow-100 rounded-full dark:bg-yellow-900 dark:text-yellow-200">Warning</span>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
    ...
    <!-- Add this after the Database Commands section -->

    <!-- Queue Management Card -->
    <div class="p-6 mt-6 bg-white border border-gray-200 shadow-sm rounded-xl dark:border-gray-700 dark:bg-gray-800">
        <div class="flex items-center gap-3 mb-4">
            <div class="flex items-center justify-center w-10 h-10 bg-orange-100 rounded-lg dark:bg-orange-900/30">
            ...
            <h4 class="text-lg font-semibold text-gray-900 dark:text-white">Queue Management</h4>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <!-- Clear Failed Jobs -->
            <button wire:click="clearFailedJobs" wire:loading.attr="disabled"
                class="inline-flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-red-700 transition border border-red-300 rounded-lg shadow-sm bg-red-50 hover:bg-red-100 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 dark:border-red-600 dark:bg-red-900/20 dark:text-red-400 dark:hover:bg-red-900/30">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm5-1a1 1 0 00-1 1v6a1 1 0 102 0V8a1 1 0 00-1-1z"
                        clip-rule="evenodd" />
            ...
            <!-- Daemon Queue Worker (with warning) -->
            <button x-data="{
                showDaemonWarning() {
                    if (confirm('⚠️ WARNING: Daemon queue workers run continuously.\n\nFor production, use Supervisor to manage queue workers.\n\nDo you want to see the command to run manually?')) {
                        $wire.call('startDaemonQueue');
                    }
                }
            }" @click="showDaemonWarning"
                class="inline-flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-purple-700 transition border border-purple-300 rounded-lg shadow-sm bg-purple-50 hover:bg-purple-100 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 dark:border-purple-600 dark:bg-purple-900/20 dark:text-purple-400 dark:hover:bg-purple-900/30">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd"
                        d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z"
                        clip-rule="evenodd" />
        ...
        <div class="pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
            <h5 class="mb-3 text-sm font-medium text-gray-900 dark:text-white">Queue Information</h5>
            <div class="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                <p>• <code class="text-xs">queue:restart</code> - Gracefully restart queue workers</p>
                <p>• <code class="text-xs">queue:retry all</code> - Retry all failed jobs</p>
                <p>• <code class="text-xs">queue:flush</code> - Delete all failed jobs</p>
                <p>• <strong>Daemon Worker:</strong> For production, use Supervisor to manage <code>queue:work
                        --daemon</code></p>
            </div>
        </div>
    </div>
