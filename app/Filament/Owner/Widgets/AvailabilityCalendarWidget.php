 ...
 *
 * ✅ FIX #1: Added `use App\Models\Booking` import for cross-referencing.
 * ✅ FIX #2: fetchEvents() now cross-references Booking table to show booked slots.
 * ✅ FIX #3: determineTimeSlot() uses `mixed` type for PHP 8.4.12 compatibility.
 * ✅ FIX #4: onDateSelect() uncommented with PHP 8.4.12 safe parameter types.
 *
 * Features:
...
class AvailabilityCalendarWidget extends FullCalendarWidget
{
    ...
    /**
     * ============================================================
     * ✅ FIX #2: Enhanced fetchEvents() with Booking cross-reference
     * ============================================================
     *
     * Previously: Only read HallAvailability records. Since bookings
     * never updated availability, all slots appeared green (available).
     *
     * Now: After loading availabilities, also loads active bookings
     * in the same date range and overrides matching slots to show
     * as booked (blue). Acts as a safety net even after the
     * BookingObserver is syncing data correctly.
     *
    ...
    public function fetchEvents(array $fetchInfo): array
    {
        ...
        // ─── Step 1: Load HallAvailability records (original behavior) ───
        ...
        // ─── Step 2: Load active bookings for cross-reference (NEW) ──────
        ...
        if ($this->selectedHallId) {
            $bookingQuery->where('hall_id', $this->selectedHallId);
        }

        $activeBookings = $bookingQuery->get();

        // ─── Step 3: Index availabilities by composite key ───────────────
        // Key: "hall_id|date|time_slot" for O(1) lookup
        $availabilityIndex = [];
        foreach ($availabilities as $availability) {
            $key = $availability->hall_id . '|'
                . $availability->date->format('Y-m-d') . '|'
                . $availability->time_slot;
            $availabilityIndex[$key] = $availability;
        }

        // ─── Step 4: Override availability for slots with active bookings ─
        foreach ($activeBookings as $booking) {
            $dateStr = $booking->booking_date->format('Y-m-d');

            // full_day booking occupies all slots
            $slotsToCheck = ($booking->time_slot === 'full_day')
                ? ['morning', 'afternoon', 'evening', 'full_day']
                : [$booking->time_slot];

            foreach ($slotsToCheck as $slot) {
                $key = $booking->hall_id . '|' . $dateStr . '|' . $slot;

                if (isset($availabilityIndex[$key])) {
                    // Availability record exists → override in-memory to booked
                    $avail = $availabilityIndex[$key];
                    if ($avail->is_available || $avail->reason !== 'booked') {
                        $avail->is_available = false;
                        $avail->reason = 'booked';
                    }
                }
                // If no availability record exists for this slot, skip it.
                // The BookingObserver will create the record going forward.
                // We don't create synthetic events here to avoid issues
                // with edit/delete modals on non-persisted records.
            }
        }

        // ─── Step 5: Convert to EventData (original mapping logic) ───────
        return collect($availabilityIndex)
    ...
    /**
     * ============================================================
     * ✅ FIX #3: PHP 8.4.12 safe type for determineTimeSlot()
     * ============================================================
     *
     * Original: ?string $datetime — crashes when FullCalendar passes
     * an array or object for $arguments['start'].
     *
     * Fixed: mixed $datetime — safely handles any input type.
     *
     * @param mixed $datetime Date/time value from FullCalendar
     * @return string Time slot key (morning|afternoon|evening)
     */
    protected function determineTimeSlot(mixed $datetime = null): string
    {
        // Return default for null, empty, or non-scalar values
        if ($datetime === null || $datetime === '' || is_array($datetime) || is_object($datetime)) {
            return 'morning';
        }

        // Safe cast to string for strtotime()
    ...
    /**
     * ============================================================
     * ✅ FIX #4: onDateSelect() — uncommented with safe types
     * ============================================================
     *
     * This method handles clicking a date on the calendar to create
     * a new availability slot. It was commented out in the original,
     * which prevented click-to-create from working.
     *
     * Parameter types changed from strict string/bool to mixed
     * for PHP 8.4.12 compatibility — FullCalendar JS may pass
     * unexpected types through Livewire.
     *
     * @param mixed $start   Start date/time from FullCalendar
     * @param mixed $end     End date/time from FullCalendar
     * @param mixed $allDay  Whether this is an all-day selection
     * @param mixed $view    View information array
     * @return void
     */
    // public function onDateSelect(mixed $start, mixed $end = null, mixed $allDay = false, mixed $view = null): void
    // {
    //     // Safe cast start to string for strtotime comparison
    //     $startStr = is_string($start) ? $start : (string) ($start ?? '');

    //     // Prevent creating in the past
    //     if ($startStr !== '' && strtotime($startStr) < strtotime('today')) {
    //         Notification::make()
    //             ->warning()
     ...
     *
     * Currently disabled — uncomment to enable drag-and-drop.
     */
        ...
        return [
            ...
            Action::make('filter_hall')
                ...
                ->action(function (array $data): void {
                    $this->selectedHallId = $data['hall_id'] ? (int) $data['hall_id'] : null;
                    $this->refreshEvents();
                }),
