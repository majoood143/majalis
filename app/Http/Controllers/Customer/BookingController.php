            ...
            if ($sessionId && ($payment->isPending() || $payment->isProcessing())) {
                ...
                try {
                    ...
                    if (isset($result['success']) && $result['success'] === true) {
                         ...
                         *
                         * Uses Notification::route() (on-demand) to send to customer_email
                         * instead of $user->notify(), because:
                         * - customer_email is always available on the booking
                         * - Works even if user relationship has issues
                         * - Mirrors the guest flow pattern for consistency
                         *
                         * The notification class handles:
                         * - PDF attachment (reuses BookingPdfService output)
                         * - Bilingual support (EN/AR)
                         * - Advance vs full payment info display
                         */
                        try {
                            ...
                            Log::info('Booking confirmation email queued', [
                                ...
                                'customer_email' => $freshBooking->customer_email,
                                'payment_type' => $freshBooking->payment_type,
                            ]);
                        } catch (Exception $e) {
                            // Email is non-critical — don't block the payment success flow
                            Log::warning('Failed to send confirmation email (non-critical)', [
                                'booking_id' => $booking->id,
                                'error' => $e->getMessage(),
                            ]);
                        }
